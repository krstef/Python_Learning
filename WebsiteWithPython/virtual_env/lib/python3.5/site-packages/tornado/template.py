...
"""A simple template system that compiles template to Python code.
...
`Loader` is a class that loads template from a root directory and caches
the compiled template::

    loader = template.Loader("/home/btaylor")
    print(loader.load("test.html").generate(myvalue="XXX"))

We compile all template to raw Python. Error-reporting is currently... uh,
interesting. Syntax for the template::
...
`.json_encode()`, and `.squeeze()` to all template by default.

Typical applications do not create `Template` or `Loader` instances by
hand, but instead use the `~.RequestHandler.render` and
`~.RequestHandler.render_string` methods of
`tornado.web.RequestHandler`, which load template automatically based
...
class BaseLoader(object):
    ...
    template after they are loaded the first time.
    """
    ...
    def __init__(
        ...
        :arg str whitespace: A string specifying default behavior for
            whitespace in template; see `filter_whitespace` for options.
        ...
        # self.lock protects self.template.  It's a reentrant lock
        # because template may load other template via `include` or
    ...
    def reset(self) -> None:
        """Resets the cache of compiled template."""
        with self.lock:
            self.templates = {}
...
def _parse(
    ...
    while True:
        ...
        # This is especially useful in conjunction with jquery template,
